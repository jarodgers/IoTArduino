...
//***************************************************************
	...
	{	
		...
		BloodPressure.begin(9600);
	 	BloodPressure.write(0xAA);
		delayMicroseconds(1);
	 	BloodPressure.write(0x55);
		delayMicroseconds(1);
		BloodPressure.write(0x88);
	 	delay(2500);

		BloodPressure.print("\n");

		if (BloodPressure.available() > 0) { // The protocol sends the measures 
			...
			for (int i = 0; i<4; i++){ // Read four dummy data	
 			...
 			while(_data != 0xD1){
  				if (ia==0){
   				_data = BloodPressure.read();
  				}

			bloodPressureDataVector[length].year = swap(_data);
			bloodPressureDataVector[length].month = swap(BloodPressure.read());
			bloodPressureDataVector[length].day = swap(BloodPressure.read());
			bloodPressureDataVector[length].hour = swap(BloodPressure.read());
			bloodPressureDataVector[length].minutes = swap(BloodPressure.read());
			bloodPressureDataVector[length].systolic = swap(BloodPressure.read());
			bloodPressureDataVector[length].diastolic = swap(BloodPressure.read());
			bloodPressureDataVector[length].pulse = swap(BloodPressure.read());
			length++;
			ia=1;
